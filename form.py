...
class CoverForm(FlaskForm):
    full_name = StringField(validators=[DataRequired()],
                            render_kw={'class': "cover", 'autocomplete': "off", "style": "width: 400px;", "onkeyup": "clear_highlight(this)"})
...
class AgreementForm(Form):
    initials = StringField(validators=[DataRequired()],
                           render_kw={'autocomplete': "off",
                                      ...
                                      'class': 'agreement', "onkeyup": onclick_clear})
...
class PersonalInformationForm(Form):
    ...
    full_legal_name = StringField(validators=[DataRequired()],
                                  render_kw={"style": style_medium, "onkeyup": onclick_clear})
    preferred_name = StringField(render_kw={"style": style_medium, "onkeyup": onclick_clear})
    pronouns = StringField(render_kw={"style": style_short, "onkeyup": onclick_clear})

    dob = DateField(format='%m/%d/%y',
                    validators=[DataRequired()],
                    render_kw={'autocomplete': "off",
                               "style": style_short,
                               "onchange": onclick_clear + ",is_underage(this)",
                               })
    ...
    phone = IntegerField(validators=[DataRequired()], render_kw={"style": style_medium, "onkeyup": onclick_clear})
    ...
    permanent_address = StringField(validators=[DataRequired()],
                                    render_kw={"style": style_long, "onkeyup": onclick_clear})

    same_address = BooleanField(render_kw={"onclick": "copy_input(this, '#permanent_address', '#current_address')"})
    ...
    move_in_term = SelectMultipleField(choices=my_choices,
                                       validators=[DataRequired()],
                                       render_kw={"style": style_short, "onchange": onclick_clear})

    move_in_year=IntegerField(validators=[DataRequired()], render_kw={"style": style_short, "onkeyup": onclick_clear})

    # SHOW AND HIDE
    was_boarder = BooleanField(render_kw={"style": style_check,
                                          'onclick': "show_hide_extra_inputs('was_boarder_checked',['was_boarder_years'])",})
    was_boarder_years = IntegerField(render_kw={"min": "1920", "max": "2022", "style": style_short, "onkeyup": onclick_clear})
    ...
    member_terminated = BooleanField(render_kw={"onclick": "show_hide_extra_inputs('was_boarder_terminated', "
                                                ...
                                                "style": style_check})
    member_terminated_explanation = TextAreaField(render_kw={"rows": "12", "cols": "150", "onkeyup": onclick_clear})
    ...
    type_student = SelectField(choices=["Non Student", "Undergrad", "Graduate", "Post-Baccalaureate"],
                               render_kw={"onchange": "hide_if_selected(this.value,"
                                          ...
                                          "style": style_short},
                               validators=[DataRequired()])
    which_school = StringField(render_kw={"style": style_medium, "onkeyup": onclick_clear})
    school_year = IntegerField(render_kw={"style": style_short, "onkeyup": onclick_clear})
    credits = IntegerField(render_kw={"style": style_short, "onkeyup": onclick_clear})
    major = StringField(render_kw={"style": "width: 200px;", "onkeyup": onclick_clear})
    ...
    how_long = StringField(render_kw={"style": style_medium, "onkeyup": onclick_clear},
                           validators=[DataRequired()])

    # SHOW AND HIDE
    pet = BooleanField(render_kw={'onclick': "show_hide_extra_inputs('has_pet', ['pet_description', 'pet_needs'])",
                                  "style": style_check})
    pet_description = StringField(render_kw={"style": style_long, "onkeyup": onclick_clear})
    pet_needs = StringField(render_kw={"style": style_long, "onkeyup": onclick_clear})
    ...
    if_other_about_us = StringField(render_kw={"style": style_long, "onchange": onclick_clear})


class ShortEssayForm(Form):
    question = TextAreaField(render_kw={"class": "essay", "rows": "12", "cols": "150", "onkeyup": onclick_clear},
                             validators=[DataRequired()])


class AutobiographicalForm(Form):
    question = TextAreaField(render_kw={"class": "auto", "rows": "12", "cols": "150", "onkeyup": onclick_clear},
                             validators=[DataRequired()])


class Range(Form):
    range = IntegerRangeField(label="range", render_kw={"value":0,
                                                        "min": 0,
                                                        "max": 10,
                                                        "step": 1,
                                                        "style": "width:500px;"})
...
class References(Form):
    name = StringField(label="Name",
                       render_kw={"class": "reference", "onkeyup": onclick_clear},
                       validators=[DataRequired()])
