...
class Application(FlaskForm):
    full_name = StringField(label='Full Name', validators=[DataRequired()],
                      render_kw={"class": "form-control p-2s",
                                 'autocomplete': "off"})
    today_date = DateTimeField(label="Today's Date", format='%m/%d/%y', validators=[DataRequired()])

    initials1 = StringField(label="Initials", validators=[DataRequired()])
    initials2 = StringField(label="Initials", validators=[DataRequired()])
    ...
    preferred_name = StringField(label="Prefered Name", validators=[DataRequired()])
    ...
    member_terminated = StringField(label="Have you ever had your membership or tenancy terminated by the SCA and/or another /" \
    ...
    type_student = SelectField(label="Type of Student", choices=["Undergrad", "Graduate", "Post-Baccalaureate"])
    school_year = IntegerField(label="School Year", validators=[DataRequired()])
    credits = IntegerField(label="Credits", validators=[DataRequired()])
    ...
    how_long =  StringField(label="How long do you expect to live in these houses? If you don’t know that’s okay.")
